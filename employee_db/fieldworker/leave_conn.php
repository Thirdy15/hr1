...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($startDate) || empty($endDate) || empty($leaveType)) {
        ...
        header("Location: ../../employee/fieldworker/leave_request.php");
        exit();
    }
    ...
    $supervisorId = 1; // Assuming supervisor ID is 1, change as needed
    $message = "New leave request submitted by employee ID: " . $employeeId;
    sendNotification($conn, $supervisorId, $message);

    $conn->close();

    header("Location: ../../employee/fieldworker/leave_file.php");
    exit();
}
?>
